...
def download_geo_file(geo_accession: str, file_type: str, output_dir: str) -> Optional[str]:
    ...
    base_url = f"https://ftp.ncbi.nlm.nih.gov/geo/series/GSE{first_two}nnn/{geo_accession}"
    ...
    if file_type == 'matrix':
        ...
        raise ValueError(f"Unknown file_type: {file_type}. Must be 'matrix', 'soft', or 'miniml'")
...
def parse_geo_series_matrix(matrix_file: str) -> Dict:
    ...
    sample_ids = expression_df.columns.tolist() if len(expression_df.columns) > 0 else []
    if not sample_ids and 'Series_sample_id' in metadata_dict:
        sample_ids = metadata_dict['Series_sample_id']
    
    metadata_df = pd.DataFrame({
        'sample_id': sample_ids,
        'geo_accession': [metadata_dict.get('Series_geo_accession', [''])[0]] * len(sample_ids) if sample_ids else []
    })
...
def extract_phenotypes(geo_data: Dict) -> pd.DataFrame:
    ...
    for key, values in raw_metadata.items():
        key_lower = key.lower()
        if 'sample' in key_lower and ('characteristic' in key_lower or 'title' in key_lower):
            for value in values:
                if ':' in value:
                    parts = value.split(':', 1)
                    if len(parts) == 2:
                        ...
                        col_name = key.replace('Series_sample_characteristic_', '').replace('Series_sample_', '').replace('_', ' ').title()
                        if col_name and col_name not in phenotype_df.columns:
                            phenotype_df[col_name] = None
                            found_cols.append(col_name)
                        if col_name:
                            idx = phenotype_df[phenotype_df['sample_id'] == sample_id].index
                            if len(idx) > 0:
...
def download_all_geo_datasets(config_path: str = "config.yaml", output_dir: str = "data/expression") -> Dict[str, Dict]:
    """

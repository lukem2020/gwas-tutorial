...
def extract_probe_to_gene_mapping(platform_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in platform_df.columns:
        ...
        if probe_col is None and ('ID' in col_upper or 'PROBE' in col_upper):
            probe_col = col
        if gene_symbol_col is None and ('GENE_SYMBOL' in col_upper or 'SYMBOL' in col_upper):
            gene_symbol_col = col
        ...
        if gene_symbol_col is None and 'GENE_ASSIGNMENT' in col_upper:
            gene_symbol_col = col
        if entrez_col is None and ('GENE' in col_upper and 'SYMBOL' not in col_upper and 'NAME' not in col_upper and 'ASSIGNMENT' not in col_upper):
            entrez_col = col
    ...
    if gene_symbol_col is None:
        raise ValueError(f"Could not find GENE_SYMBOL or gene_assignment column in platform table. Available columns: {list(platform_df.columns)}")
    ...
    if 'ASSIGNMENT' in gene_symbol_col.upper():
        ...
        def extract_gene_symbols(assignment_str):
            ...
            unique_symbols = list(dict.fromkeys(gene_symbols))  # Preserve order, remove duplicates
            return '; '.join(unique_symbols) if unique_symbols else ''
        
        gene_symbols_parsed = platform_df[gene_symbol_col].astype(str).apply(extract_gene_symbols)
...
def aggregate_probes_to_genes(expression_df: pd.DataFrame, 
    ...
    print(f"      Probes per gene: min={probes_per_gene.min()}, "
          f"max={probes_per_gene.max()}, mean={probes_per_gene.mean():.2f}")
    
    return gene_expression


def merge_expression_with_phenotypes(gene_expression: pd.DataFrame,
    ...
    if len(common_samples) == 0:
        raise ValueError("No matching samples found between expression and phenotype data")
    ...
    phenotypes_filtered = phenotypes_df[phenotypes_df[sample_id_col].astype(str).isin(common_samples)].copy()
    ...
    # Merge
    combined_df = expression_t.merge(
        phenotypes_filtered,
        on=sample_id_col,
        how='inner'
    )
    
    print(f"      Merged dataset: {len(combined_df)} samples × {len(gene_expression_filtered)} genes + phenotypes")
...
def process_geo_dataset(geo_accession: str,
    ...
    if missing_files:
        raise FileNotFoundError(f"Missing files for {geo_accession}: {', '.join(missing_files)}")
    ...
    gene_expression = aggregate_probes_to_genes(expression_df, probe_to_gene, aggregation_method)
    ...
    stats = {
        ...
        'probes_per_gene_mean': len(expression_df) / len(gene_expression) if len(gene_expression) > 0 else 0
    }
...
def save_processed_data(results: Dict, geo_accession: str, output_dir: str = "data/expression") -> Dict[str, Path]:
    """
...
def process_all_datasets(config_path: str = "config.yaml",
                         data_dir: str = "data/expression",
                         aggregation_method: str = 'mean') -> Dict[str, Dict]:
    """

...
try:
    from src.utils.real_data_loader import quick_load_real_data
    ...
    print("\nDownload it first:")
    print("  python -c \"from src.utils.real_data_loader import RealDataLoader; RealDataLoader().download_1000genomes(22)\"")
    sys.exit(1)
    ...
    for i in range(genotypes.shape[1]):
        ...
        try:
            ...
            chi2, p = stats.chi2_contingency([
                [sum((phenotype == 0) & (snp_clean == 0)), sum((phenotype == 0) & (snp_clean > 0))],
                [sum((phenotype == 1) & (snp_clean == 0)), sum((phenotype == 1) & (snp_clean > 0))]
            ])[:2]
...
if np.sum(valid_idx) > 0:
    ...
    print("\nTop 5 Associations:")
    print(f"{'Rank':<6} {'SNP':<15} {'Chr':<5} {'Position':<12} {'P-value':<15} {'Beta':<10}")
    print("-"*70)
    
    for rank, idx in enumerate(actual_indices, 1):
        print(f"{rank:<6} {snp_info.iloc[idx]['SNP']:<15} "
...
if len(sig_indices) > 0:
    ...
    for q in ['Q1', 'Q2', 'Q3', 'Q4', 'Q5']:
        ...
        print(f"    {q}: OR = {or_val:.2f} ({cases_in_q} cases, {controls_in_q} controls)")
else:
...
try:
    ...
    plt.plot([0, -np.log10(expected).max()], [0, -np.log10(expected).max()], 
             'r--', label='y=x')

...
def find_variants_in_region(vcf_path: str, region: str, use_index: bool = True) -> List[int]:
    """
    ...
    if ':' in region:
        ...
        raise ValueError(f"Invalid region format: {region}. Use 'chr:start-end' or 'chr:pos'")
    ...
    indices = [
        i for i, (c, p) in enumerate(zip(chroms, positions))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if not vcf_path:
            print("Usage: python -m src.utils.vcf_indexer <path_to_vcf> [region] [--force]")

...
def create_binary_disease_status(combined_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in combined_df.iterrows():
        ...
        if "disease_status" in combined_df.columns:
            status_text = (
                str(row["disease_status"]).lower()
                if pd.notna(row["disease_status"])
                else ""
            )
            if any(
                term in status_text
                for term in ["ipf", "idiopathic pulmonary fibrosis", "disease", "case"]
            ):
        ...
        if disease_status_val is None and "disease_state" in combined_df.columns:
            status_text = (
                str(row["disease_state"]).lower()
                if pd.notna(row["disease_state"])
                else ""
            )
            if any(
                term in status_text
        ...
        if disease_status_val is None:
            ...
            if any(
                term in desc or term in title for term in ["ipf", "disease", "case"]
            ):
                disease_status_val = 1
            elif any(
                term in desc or term in title
                for term in ["control", "healthy", "normal", "n_"]
            ):
                disease_status_val = 0
    ...
    combined_df["disease_status"] = pd.Series(
        disease_status_binary, dtype="Int64"
    )  # Int64 allows NaN
...
def clean_geo_dataset(geo_accession: str, data_dir: str = "data/expression") -> Dict:
    ...
    phenotype_cols = [
        ...
        if col in [k.replace(" ", "_").lower() for k in all_phenotype_keys]
    ]
    ...
    stats = {
        ...
        "disease_status_1": (
            sum(combined_df["disease_status"] == 1)
            if "disease_status" in combined_df.columns
            else 0
        ),
        "disease_status_0": (
            sum(combined_df["disease_status"] == 0)
            if "disease_status" in combined_df.columns
            else 0
        ),
        "disease_status_missing": (
            sum(combined_df["disease_status"].isna())
            if "disease_status" in combined_df.columns
            else 0
        ),
    }
...
def clean_all_datasets(
    config_path: str = "config.yaml", data_dir: str = "data/expression"
) -> Dict[str, Dict]:
    """
...
def main():
    ...
    try:
        ...
        print(
            "  - {GSE}_cleaned.csv (cleaned dataset with extracted phenotypes and binary disease_status)"
        )
